...
class mSum:
    ...
    def __repr__(self):
        ...
        for i in range(len(self.block[0])):
            elem = self.block[0, i]
            result_str += elem.sign  # otherwise the '+' won't be printed
            unsigned_elem = copy(elem)
            unsigned_elem.sign = '+'
            result_str += "  " + str(unsigned_elem) + "   "
        return result_str
...
class Block(np.ndarray):
    ...
    def __new__(cls, block_ini="x_{1} & x_{2} // -x_{2}* & x_{1}*"):
        """ Builds a new Block.
        using 2dim list : builds a Block by concatenating each sub-list horizontally and then concatenating all
                          the results vertically.
        using 'block_string' : According to the block format, rows are separated by '//' and elements by '&'"""
        # If block_ini is not a string
        if not isinstance(block_ini, str):
            if isinstance(block_ini, list) and\
                    any(isinstance(list_i, list) for list_i in block_ini):
                return np.vstack([np.hstack(list_i) for list_i in block_ini]).view(Block)
            else:
                raise TypeError("block_ini must be 2 dimensional list!")
        # else use block_string
